...
<h3>Timeline</h3>
<p>Two months of build, then a twelve-month maintenance window. The compressed schedule is realistic because the Testnet implementation already exists: the research phase starts from working code and a findings log, not a blank repository.</p>
<table>
  <thead>
    <tr><th>Period</th><th>Focus</th><th>Deliverables</th></tr>
  </thead>
  <tbody>
    <tr><td>Days 1-15</td><td>Compatibility research</td><td>Test harness and device runbook published. <code>docs/compatibility-matrix.md</code> and <code>docs/usage-patterns.md</code> tested on physical devices across the full coverage table in Section 3, with findings log. Matrix rows carry <em>Last tested</em> dates.</td></tr>
    <tr><td>Days 16-35</td><td>Core SDK + UI components</td><td><code>passkey-sdk</code> core package with full Vitest suite; <code>passkey-ui</code> Web Component set (<code>&lt;pk-create&gt;</code>, <code>&lt;pk-sign-tx&gt;</code>, <code>&lt;pk-recover&gt;</code>). All packages published to npm under MIT license.</td></tr>
    <tr><td>Days 36-55</td><td>stellar-wallets-kit integration + delivery</td><td><code>wallets-kit-adapter</code> implementing the <code>IStellarWalletsKit</code> module interface. PR raised against the official stellar-wallets-kit repo. Integration tested with Meridian Pay and Freighter teams. End-to-end demo live on Testnet. Public blog post on compatibility learnings. Docs, matrix, and harness merged into the stellar-wallets-kit repo.</td></tr>
    <tr><td>Months 3-14</td><td>Maintenance window</td><td>Quarterly physical re-tests and per-major-browser-release updates. CI virtual-authenticator suite stays green. Out-of-cycle update PRs within two weeks of any breaking change in WebAuthn platform APIs or the reference contracts. Support for wallet teams integrating the adapter.</td></tr>
  </tbody>
</table>
