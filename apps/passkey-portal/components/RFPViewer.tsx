...
<p>Passkeys solve this and many teams are trying to implement them on Stellar. Already billions of users authenticate with passkeys created using Apple ID and Google accounts to access their banks, storage, and much more. They require no install, no seed phrase, and no new mental model. Stellar's Protocol 21 added native support for the cryptography passkeys use. What is missing is a <strong>documented, reusable, production-quality SDK and UI layer</strong> integrated with stellar-wallets-kit that Stellar wallet developers can actually ship.</p>
...
<h2>4. Passkey Compatibility & Platform Behaviour</h2>
...
<p>Our deliverable uses smart-account-kit as a reference implementation to extract the minimum viable WebAuthn and Soroban payload logic into a new, standalone SDK, then adds the UI and adapter layer on top. The two projects are complementary.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td><strong>M3</strong> stellar-wallets-kit Integration</td><td><code>wallets-kit-adapter</code> implementing the full <code>IStellarWalletsKit</code> module interface (<code>getAddress</code>, <code>signTransaction</code>, <code>isAvailable</code>).PR raised against the official stellar-wallets-kit repo. Docs and compatibility matrix merged into the repo under a community-owned path so the ecosystem retains the material after the grant. Integration validated with Meridian Pay and Freighter teams. End-to-end demo live on Testnet. Public blog post on passkey compatibility learnings. First-year maintenance commitment: quarterly re-tests + out-of-cycle update PRs within two weeks of any breaking change in smart-account-kit or WebAuthn platform APIs.</td><td>PR open and passing CI in stellar-wallets-kit repo; passkey option appears alongside Freighter and Lobstr in the kit; integration confirmed by at least one wallet team; demo URL live and walkthrough-able; blog post published; maintenance commitment documented in repo <code>CONTRIBUTING.md</code></td><td><strong>$30,000</strong></td></tr>
  </tbody>
</table>
`
